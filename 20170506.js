...
function loadAccount() {
  new Promise(function(resolve, reject) { 
    ...
    if(api.isConnected()) resolve();
      ...
      console.log('Disconnected in loadAccount, code:', code);
      try {
        ...
        console.log("Failed to reconnect in loadAccount.");
        resolve();
      }
    }
    }).then(function() { 
    ...
    }, function(err) { console.log("Error getAccountInfo: "+err); }).then(function(info) {
      if(info) {
  ...
  }, function (err) { console.log("Error getTrustlines: "+err); }).then(function(lines) {
...
function loadOrderbook() {
  ...
  new Promise(function(resolve, reject) {
    if(api.isConnected()) resolve();
      ...
      console.log('Disconnected in loadOrderbook, code:', code);
      try {
        api.connect().then(function() {
        ...
        console.log("Failed to reconnect in loadOrderbook.");
        resolve();
      }
    }
  }).then(function() {
    if(showOrderbook && getSelectedText()=="") updateSymbols();
  }).then(function() { if(showOrderbook && getSelectedText()=="" && symbol1.length==3 && symbol2.length==3) {
    try {
    ...
    if(showOrderbook && orderbook!=null) {
      ...
      var cols = 6;
      bidasktable += "<table><tr><td colspan='"+cols+"' style='text-align:left; width:40%;'>Bids</td><td style='width:20%; text-align:center;'><a href='?action="+(action=="sell"? "buy":"sell")+"&amp;qty1="+$("#price").val()+"&amp;symbol1="+symbol2+(issuer2==""? "":"."+issuer2)+"&amp;qty2="+$("#qty1").val()+"&amp;symbol2="+symbol1+(issuer1==""? "":"."+issuer1)+"' target='_blank' onclick='loadURLSymbols(\""+(action=="sell"? "buy":"sell")+"\", $(\"#price\").val(), \""+symbol2+(issuer2==""? "":"."+issuer2)+"\", $(\"#qty1\").val(), \""+symbol1+(issuer1==""? "":"."+issuer1)+"\"); return false;'>Switch</a> </td><td colspan='"+cols+"' style='text-align:right; width:40%;'>Asks</td></tr>";
      ...
      for(var i=0; i<Math.max(orderbook.bids.length, orderbook.asks.length); i++) {
        ...
        if(i< orderbook.asks.length && orderbook.asks[i].specification.quantity.value!=0) {
          ...
          if(orderbook.asks[i].state!=null && orderbook.asks[i].state.fundedAmount!=null && orderbook.asks[i].state.fundedAmount.value>0) {
             ...
             q1=orderbook.asks[i].state.fundedAmount.value / ask;
                ...
                api.preparePayment(address, payment, options).then(function(prepared)
                  {
                    ...
                    if(transaction!="") {
                      api.submit(transaction).then(function(result) {
                        ...
                        else $("#errors").html("Submitted with result: "+result.resultMessage+" <a href='https://charts.ripple.com/#/transactions/"+transactionID+"' target='_blank'>See Details...</a><br />Check <a href='https://charts.ripple.com/#/graph/"+address+"' target='_blank'>Account History</a> to confirm successful transaction.");
                        errored = true;
                        loadAccount();
                        refreshLayout();
                      }, function (err) {
            ...
            api.prepareOrder(address, order, options).then(function(prepared)
              {
                ...
                if(transaction!="") {
                  api.submit(transaction).then(function(result) {
                    ...
                    else $("#errors").html("Submitted with result: "+result.resultMessage+". <a href='https://charts.ripple.com/#/transactions/"+transactionID+"' target='_blank'>See Details...</a><br />Check <a href='https://charts.ripple.com/#/graph/"+address+"' target='_blank'>Account History</a> to confirm order or trade.");
                    errored = true;
                    loadAccount();
                    refreshLayout();
                  }, function (err) {
...
$(document).ready(function() {
    ...
    api.on('tooBusy', (code) => {
      // code - [close code](https://developer.mozilla.org/en-US/docs/Web/API/CloseEvent) sent by the server
      // will be 1000 if this was normal closure
      console.log('Error tooBusy, code:', code);
    });
    api.on('noNetwork', (code) => {
      // code - [close code](https://developer.mozilla.org/en-US/docs/Web/API/CloseEvent) sent by the server
      // will be 1000 if this was normal closure
      console.log('Error noNetwork, code:', code);
    });
