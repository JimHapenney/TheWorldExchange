...
function loadAccount() {
  new Promise(function(resolve, reject) { 
    ...
    }, function(err) { console.log("Error getBalances: "+err); }).then(function(balances) {
        var balanceOutput = "";
        holdings = {};
  ...
  }, function(err) {console.log("Error getOrders: "+err); }).then(function(orders) {
    var ordersOutput = "";
    if(address!="" && orders) {
  ...
  }, function(err) { console.log("Error completing account load: "+err); });
}
...
function loadOrderbook() {
  ...
  new Promise(function(resolve, reject) {
    ...
    try {
      ...
      return api.getOrderbook(address=="" || address.length<=10?  Object.keys(issuerNames)[0]:address, getPair(), {limit:20}); 
    }
    catch (ex) {
      showOrderbook = false;
      if(action!="issue") {
        $("#errors").html("No orderbook for symbols "+symbol1+" / "+symbol2+" found. Check spelling or issuer/backer.<br />("+ex+")");
        errored = true;
      }
      refreshLayout();
    }
    ...
    if(showOrderbook && orderbook!=null) {
      ...
      var cols = 6;
      bidasktable += "<table><tr><td colspan='"+cols+"' style='text-align:left; width:40%;'>Bids</td><td style='width:20%; text-align:center;'><a href='?action="+(action=="sell"? "buy":"sell")+"&amp;qty1="+($("#qty1").val()*$("#price").val())+"&amp;symbol1="+symbol2+(issuer2==""? "":"."+issuer2)+"&amp;price="+(1/$("#price").val())+"&amp;symbol2="+symbol1+(issuer1==""? "":"."+issuer1)+"' target='_blank' onclick='loadURLSymbols(\""+(action=="sell"? "buy":"sell")+"\", $(\"#qty1\").val()*$(\"#price\").val(), \""+symbol2+(issuer2==""? "":"."+issuer2)+"\", 1/$(\"#price\").val(), \""+symbol1+(issuer1==""? "":"."+issuer1)+"\"); return false;'>Switch</a> </td><td colspan='"+cols+"' style='text-align:right; width:40%;'>Asks</td></tr>";
      ...
      for(var j=0; j<Math.min(bookdepth, Math.max(bids.length, asks.length)); j++) {
        var bidurl = j>=bids.length? "":"<a target='_blank' href='?action=sell&amp;qty1="+(bids[j].qty)+"&amp;symbol1="+bids[j].symbol1complete+"&amp;price="+bids[j].price+"&amp;symbol2="+bids[j].symbol2complete+"' onclick='loadURLSymbols(\"sell\", "+bids[j].qty+", \""+bids[j].symbol1complete+"\", "+bids[j].price+", \""+bids[j].symbol2complete+"\"); return false;'>";
        var bidurlprice = j>=bids.length? "":"<a target='_blank' href='?action=sell&amp;qty1="+(bids[j].qty)+"&amp;symbol1="+bids[j].symbol1complete+"&amp;price="+bids[j].price+"&amp;symbol2="+bids[j].symbol2complete+"' onclick='loadURLSymbols(\"sell\", "+bids[j].qty+", \""+bids[j].symbol1complete+"\", "+bids[j].price+", \""+bids[j].symbol2complete+"\"); return false;'>";
        var askurl = j>=asks.length? "":"<a target='_blank' href='?action=buy&amp;qty1="+(asks[j].qty)+"&amp;symbol1="+asks[j].symbol1complete+"&amp;price="+asks[j].price+"&amp;symbol2="+asks[j].symbol2complete+"' onclick='loadURLSymbols(\"buy\", "+asks[j].qty+", \""+asks[j].symbol1complete+"\", "+asks[j].price+", \""+asks[j].symbol2complete+"\"); return false;'>";
        var askurlprice = j>=asks.length? "":"<a target='_blank' href='?action=buy&amp;qty1="+(asks[j].qty)+"&amp;symbol1="+asks[j].symbol1complete+"&amp;price="+asks[j].price+"&amp;symbol2="+asks[j].symbol2complete+"' onclick='loadURLSymbols(\"buy\", "+asks[j].qty+", \""+asks[j].symbol1complete+"\", "+asks[j].price+", \""+asks[j].symbol2complete+"\"); return false;'>";
      ...
      bidasktable+="<tr><td colspan='"+(cols)+"' style='text-align:left; width:40%; border-width:0px;'>"+(mktcap1>0? "Total "+symbol1+" Issued: "+nFormatter(mktcap1, 2):"")+"</td><td style='border-width:0; width:20%; text-align:center;'>"+new Date(Date.now()).toLocaleTimeString()+"</td><td colspan='"+(cols)+"' style='text-align:right; width:40%; border-width:0px;'>"+(mktcap2>0? "Total "+symbol2+" Issued: "+nFormatter(mktcap2, 2):"")+"</td></tr>";
