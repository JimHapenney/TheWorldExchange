  ...
  // Work in progress for autobridged orderbooks
    ...
    if(!errored) {
      if(action!="issue" && action!="send" && $.trim( $('#orderbook').html() ).length) {
        ...
        refreshLayout();
          ...
          errored=true;
          $("#errors").html("Send to others by inputting their account address above.<br /><br />To receive or let others send to you, share the below link:<br /><input type='text' value='https://www.theworldexchange.net/?action=send&amp;recipient="+address+"' onclick='this.select();' readonly='readonly' class='linkShare' /><br /><br />See: <a href='#started_receivableTokensList' onclick='document.getElementById(\"about\").style.display=\"block\"; setURL(\"#started_receivableTokensList\"); jQuery(\"html,body\").animate({scrollTop: jQuery(\"#started_receivableTokensList\").offset().top}, 1000); return false;'>How to Get Started Sending and Receiving</a>");
          refreshLayout();
      }
      else refreshLayout();
    }
    ...
    if(!updateMessageOnly && showOrderbook && orderbook!=null && orderbookExists) {
      ...
      for(var j=0; j<Math.min(bookdepth, Math.max(bids.length, asks.length)); j++) {
        ...
        var bidurl = j>=bids.length? "":"<a target='_blank' href='?action=sell&amp;symbol1="+bids[j].symbol1complete+"&amp;price="+parseFloat(parseFloat(bids[j].price)-0.00001)+"&amp;symbol2="+bids[j].symbol2complete+"' onclick='loadURLSymbols(\"sell\", "+bids[j].qty+", \""+bids[j].symbol1complete+"\", "+parseFloat(parseFloat(bids[j].price)-0.00001)+", \""+bids[j].symbol2complete+"\"); return false;'>";
        var bidurlprice = j>=bids.length? "":"<a target='_blank' href='?action=sell&amp;qty1="+(bids[j].qty)+"&amp;symbol1="+bids[j].symbol1complete+"&amp;price="+parseFloat(parseFloat(bids[j].price)-0.00001)+"&amp;symbol2="+bids[j].symbol2complete+"' onclick='loadURLPrice(\"sell\", \""+bids[j].symbol1complete+"\", "+parseFloat(parseFloat(bids[j].price)-0.00001)+", \""+bids[j].symbol2complete+"\"); return false;'>";
        var askurl = j>=asks.length? "":"<a target='_blank' href='?action=buy&amp;qty1="+(asks[j].qty)+"&amp;symbol1="+asks[j].symbol1complete+"&amp;price="+parseFloat(parseFloat(asks[j].price)+0.00001)+"&amp;symbol2="+asks[j].symbol2complete+"' onclick='loadURLSymbols(\"buy\", "+asks[j].qty+", \""+asks[j].symbol1complete+"\", "+parseFloat(parseFloat(asks[j].price)+0.00001)+", \""+asks[j].symbol2complete+"\"); return false;'>";
        var askurlprice = j>=asks.length? "":"<a target='_blank' href='?action=buy&amp;symbol1="+asks[j].symbol1complete+"&amp;price="+parseFloat(parseFloat(asks[j].price)+0.00001)+"&amp;symbol2="+asks[j].symbol2complete+"' onclick='loadURLPrice(\"buy\", \""+asks[j].symbol1complete+"\", "+parseFloat(parseFloat(asks[j].price)+0.00001)+", \""+asks[j].symbol2complete+"\"); return false;'>";
    ...
    api.prepareTrustline(address, line, options).then(function(prepared)
    {
        ...
        if(transaction!="") {
          api.submit(transaction).then(function(result) {
            ...
            if(result.resultCode=="tesSUCCESS") $("#popupText").append("<br /> - Completed update for "+symbol+" by "+backer+". (See <a href='https://charts.ripple.com/#/transactions/"+transactionID+"' target='_blank'>transaction details</a>)");
            ...
            else $("#popupText").append("<br /> - Error for adding "+symbol+" by "+backer+" ("+result.resultCode+"): "+result.resultMessage+" (See <a href='https://charts.ripple.com/#/transactions/"+transactionID+"' target='_blank'>transaction details</a>)");
...
function runChat() {
  ...
  if(showChat) { // download chat only if the box is showing and enabled
    ...
    .then(function (transactions) {
      ...
      if(!firstRun && !chatLoaded) chatLoaded = true;
