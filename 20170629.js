...
function loadOrderbook() {
  ...
  new Promise(function(resolve, reject) {
    ...
    if(!errored) {
      if(action!="issue" && $.trim( $('#orderbook').html() ).length) {
        $("#errors").html("&nbsp;");
        refreshLayout();
      }
    ...
    if(showOrderbook && orderbook!=null) {
      ...
      bidasktable+="<tr><td colspan='"+(cols)+"' style='text-align:left; border-width:0px;'>Total "+symbol2+" Issued: "+nFormatter(mktcap2, 2)+"</td><td colspan='1' style='border-width:0; text-align:center; overflow:hidden;'>"+new Date(Date.now()).toLocaleTimeString('en-GB')+"</td><td colspan='"+(cols)+"' style='text-align:right; border-width:0px;'>Total "+symbol1+" Issued: "+nFormatter(mktcap1, 2)+"</td></tr>";
