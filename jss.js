...
var issuers = {
    ...
    "USD": ["rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq", "rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B", "rMwjYedjc7qqtKYVLiAccJSmCwih4LnE2q"],
  ...
  new Promise(function(resolve, reject) { 
    ...
    if(address!="" && orders) {
      for(var i=0; i<orders.length; i++) {
        ...
        ordersOutput+="<span style='whitespace:no-wrap;'><a href='#' onclick='cancelOrder("+orderSeq+");'>[X]</a> <a target='_blank' href='?action="+direction+"&amp;qty1="+qty+"&amp;symbol1="+s1+"&amp;price="+price+"&amp;symbol2="+s2+"' onclick='loadURLSymbols(\""+direction+"\", "+qty+", \""+s1+"\", "+price+", \""+s2+"\"); return false;'>"+direction+" "+parseFloat(qty.toFixed(qty>1? 2:4)).toString()+" "+symbol1+" @ "+parseFloat(price.toFixed(price>1? 2:4)).toString()+" "+symbol2+"</a></span>";
      }
    }
    if(ordersOutput!="") {
...
function refreshLayout() {
  ...
  if($("#issuer2Width").length && Math.abs($("#counterparty").width()-$("#issuer2Width").width())>5) 
    $("#issuer2Width").css("width", $("#counterparty").width()+"px");
  
  $('#footer').css('margin-top', Math.max(10, (($('#container').height()-$('#content').height()-$('#footer').height()-20)))+'px');
}
  ...
  if(!address || !key) {
    ...
    if(trans=="send") {
        ...
        if(qty<=0 || !$.isNumeric(qty)) {
          ...
          if(symbol1==baseCurrency) {
            ...
            if(!trusted && issuer1!=recipient) {
              ...
              try {
                //console.log(payment);
                api.preparePayment(address, payment).then(function(prepared)
                  {
                    var transaction = "";
                    var transactionID = -1;
                    try {
                    ...
                    if(transaction!="") {
                      api.submit(transaction).then(function(result) {
                        if(result.resultCode=="tesSUCCESS")
                          $("#errors").html(qty+" "+$("#symbol1").val()+" sent to "+recipient+"!");
                        else $("#errors").html("Submitted with result: "+result.resultMessage);
                        errored = true;
                        loadAccount();
                        refreshLayout();
                      }, function (err) {
        ...
        if(qty1<=0 || !$.isNumeric(qty1)) {
          ...
          try {
            //console.log(order);
            api.prepareOrder(address, order).then(function(prepared)
              {
                var transaction = "";
                var transactionID = -1;
                try {
...
$(document).ready(function() {
    document.title="The World Exchange - Trade, Issue, and Send Anything Anywhere in Real Time";
    $("h1").html("The World Exchange");
    $("h2").html("Trade, Issue, and Send Anything Anywhere in Real Time");
    $("#footer").html("© 2016 <a href='https://www.theworldexchange.net/'>The World Exchange</a> | <a target='_blank' href='mailto:contact@theworldexchange.net'>Contact@TheWorldExchange.Net</a> | Follow: <a target='_blank' href='https://twitter.com/WorldExchangeX'>@WorldExchangeX</a><br />Created by <a href='https://www.autodidactic.ai/' target='_blank'>Autodidactic I</a><br />Powered by <a target='_blank' href='https://en.wikipedia.org/wiki/Ripple_(payment_protocol)'>Ripple</a>");
