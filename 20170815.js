  ...
  */
    ...
    if(!updateMessageOnly && showOrderbook && orderbook!=null && orderbookExists) {
      ...
      for(var j=0; j<Math.min(bookdepth, Math.max(bids.length, asks.length)); j++) {
        ...
        var askurlprice = j>=asks.length? "":"<a target='_blank' href='?action=buy&amp;symbol1="+asks[j].symbol1complete+"&amp;price="+parseFloat(parseFloat(asks[j].price)+0.0000001)+"&amp;symbol2="+asks[j].symbol2complete+"' onclick='loadURLPrice(\"buy\", \""+asks[j].symbol1complete+"\", "+parseFloat(parseFloat(asks[j].price)+0.0000001)+", \""+asks[j].symbol2complete+"\"); return false;'>";
...
function printChat(msg) {
  var scrolled = $('#chatHistoryContents').scrollTop()+$('#chatHistoryContents').innerHeight()>=$('#chatHistoryContents')[0].scrollHeight;
...
function refreshChatLayout() {
  var scrollDown = $('#chatHistoryContents').scrollTop()+$('#chatHistoryContents').innerHeight()>=$('#chatHistoryContents')[0].scrollHeight;
