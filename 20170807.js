...
var chatDefaultMarginRight = 15; // for resetting it to the corner
var chatDefaultMarginBottom = 15;
...
function loadAccount(loadOrderbookNext=false) {
  ...
  new Promise(function(resolve, reject) { 
    ...
    var commonMenu = "<div id='chatHide' "+(showChat? "":"style='display:none;'")+"><a href='#' onclick='hideChatbox();'>Hide Chat</a></div><div id='chatShow' "+(!showChat? "":"style='display:none;'")+"><a  href='#' onclick='showChatbox();'>Show Chat</a></div>";
...
function showChatbox(updateMenu = true) {
  ...
  if(updateMenu) { // not built until end of doc load
...
function hideChatbox(updateMenu = true) {
  ...
  if(updateMenu) {//not built until end of doc load
    document.getElementById("chatShow").style.display="block"; 
...
function rescaleWindow(resizeChat=true) {
  $('#container').css('height', Math.floor($(window).height()*.98)+'px');
...
function runChat() {
  ...
  if(showChat) { // download chat only if the box is showing and enabled
    ...
    if(firstRun) printChat("Retrieving chat messages...");
...
function chatFullScreen() {
  if(isMobile()) {
    ...
    $("#chatbox").css("left", 0);
    $("#chatbox").css("top", $("#errors").offset().top+30);
  }
  else {
    $("#chatbox").css("width", $("#submit").offset().left + $("#submit").width() - $("#action").offset().left);
    $("#chatbox").css("height", $("#footer").offset().top+$("#footer").height()*.85 - $("#errors").offset().top-30);
...
function chatIsFullScreen() {
  if(isMobile())
    return $("#chatbox").width()*1.1>=$(window).width()-40 
    && $("#chatbox").height()*1.1>= $("#footer").offset().top+$("#footer").height()*.75 - $("#orderbook").offset().top;
  else
    return $("#chatbox").width()*1.1>=$("#submit").offset().left + $("#submit").width() - $("#action").offset().left 
    && $("#chatbox").height()*1.1>= $("#footer").offset().top+$("#footer").height()*.75 - $("#orderbook").offset().top;
}
...
$(document).ready(function() {
    ...
    $("#chatMessage").on( "click", function() { $('#chatHistory').scrollTop($('#chatHistory')[0].scrollHeight); });
    ...
    if(!isMobile()) {
      ...
      // Fill the space on the lower right
      var originalWidth = $("#chatbox").width();
      var originalLeft = $("#chatbox").offset().left;
      var originalHeight = $("#chatbox").height();
      var originalTop = $("#chatbox").offset().top;
      
      chatDefaultMarginRight = $("#container").width()-originalWidth-originalLeft;
      chatDefaultMarginBottom = windowHeight()-originalHeight-originalTop;
      ...
      // Irrelevant to mobile, which is already always maxed out
        $("#chatbox").draggable().resizable({ handles: 'all' }).on('resize', function(e) { e.stopPropagation(); } );
    ...
    try {
      ...
      // Least important stuff go last
          ...
          $.get( dataAPI+"/v2/gateways/", function( data ) {
            ...
            $("#chatHistoryContents").append(" Each message costs a fraction of "+baseCurrency+" ("+nFormatter(fee+0.000001, accuracy)+") due to transaction cost of the network but nothing more. Display names will be added in next update.<br /><hr />");
            runChat();
          ...
          $(window).scroll(function(e) {
            ...
            if($("#about").css("display")=="block") {
               ...
               if (!scrolling && numIntervals>=0 && wS == 0){
                 $("#about").css("display", "none");
