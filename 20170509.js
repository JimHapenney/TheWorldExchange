...
function loadOrderbook() {
  ...
  new Promise(function(resolve, reject) {
    ...
    if(showOrderbook && orderbook!=null) {
      ...
      var cols = 6;
      bidasktable += "<table><tr><td colspan='"+cols+"' style='text-align:left; width:45%;'>Bids</td><td style='width:10%; text-align:center; overflow:hidden;'><a href='?action="+(action=="sell"? "buy":"sell")+"&amp;qty1="+($("#qty1").val()*$("#price").val())+"&amp;symbol1="+symbol2+(issuer2==""? "":"."+issuer2)+"&amp;price="+(1/$("#price").val())+"&amp;symbol2="+symbol1+(issuer1==""? "":"."+issuer1)+"' target='_blank' onclick='loadURLSymbols(\""+(action=="sell"? "buy":"sell")+"\", $(\"#qty1\").val()*$(\"#price\").val(), \""+symbol2+(issuer2==""? "":"."+issuer2)+"\", 1/$(\"#price\").val(), \""+symbol1+(issuer1==""? "":"."+issuer1)+"\"); return false;'>Switch</a> </td><td colspan='"+cols+"' style='text-align:right; width:45%;'>Asks</td></tr>";
      ...
      bidasktable+="<tr><td colspan='"+(cols-1)+"' style='text-align:left; border-width:0px;'>Total "+symbol1+" Issued: "+nFormatter(mktcap1, 2)+"</td><td colspan='3' style='border-width:0; text-align:center; overflow:hidden;'>"+new Date(Date.now()).toLocaleTimeString()+"</td><td colspan='"+(cols-1)+"' style='text-align:right; border-width:0px;'>Total "+symbol2+" Issued: "+nFormatter(mktcap2, 2)+"</td></tr>";
